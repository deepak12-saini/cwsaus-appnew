	<div class="page-content">
		<div class="page-header">
			...
			<?php echo $this->Html->link('Add New',array('controller' => 'natas','action' => 'addevent'),array('class'=>'btn btn-primary btn-xs top-button')); ?>	
			</h1>
		</div>
	...
	<div class="row">
		<div class="col-xs-12">
			<?php
				...
				$nextyear = $nextdate+1;
			?>
			<a href="<?php echo SITEURL.'natas/index/'.$lastyear;?>" class="btn btn-mini btn-info "><< <?php echo $lastyear; ?> </a> 
			
			<a href="<?php echo SITEURL.'natas/index/'.$nextyear;?>" class="btn btn-mini btn-inverse" style="float:right;">>> <?php echo $nextyear; ?> </a>
...
function openpoup(id){
	var url = '<?php echo SITEURL.'natas/update/' ?>'+id;
	$('#abc_frame').attr('src', url);
	$("#myModalNew").trigger('click');	
}

function closeModal(){
	window.location.href = '<?php echo SITEURL.'natas' ?>';
}
